...
{ 
    ...
    while (nh.ok()){ 
        ...
        try{          
            listener.lookupTransform("/freenect2_depth_frame", "/torso_1", ros::Time(0), tf); 
	   // listener.lookupTransform("/openni_depth_frame", "/torso_2", ros::Time(0), tf);  
	    ...
	    double pose_x2 = tf.getOrigin().x();  
            ...
            ROS_INFO("实时位置: [x,y,theta]: [%f, %f, %f]...",pose_x,pose_y,pose_theta);  
            listener.lookupTransform("/freenect2_depth_frame", "/right_hand_1", ros::Time(0), tf_right_hand);
            double tf_right_hand_z = tf_right_hand.getOrigin().z();  
            listener.lookupTransform("/freenect2_depth_frame", "/right_shoulder_1", ros::Time(0), tf_right_shoulder);
            double tf_right_shoulder_z = tf_right_shoulder.getOrigin().z();  
        }  
        catch (tf::TransformException& ex){  
            ROS_ERROR("正在获取信息: %s", ex.what());
	...
	double tf_right_shoulder_z = tf_right_shoulder.getOrigin().z();   
        ...
        {  
            ...
            cmd.angular.z = 0.0;  
            pub.publish(cmd);  
            ROS_INFO("设定速度和方向: [v_x,v_y,w]:[%f, %f, %f]", cmd.linear.x, cmd.linear.y, cmd.angular.z);  
            const char *str1 = "停止";   
            loop_rate.sleep();  
            continue;  
        }  
